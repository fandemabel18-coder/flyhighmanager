...
function saveAccordion(){
  ...
  function quickPlace(p){
    const LIB_SLOT = 6; // idx lógico reservado para el Líbero

    // 1) Intentar colocarlo directamente en cancha
    let targetIdx = -1;
    if (p.posicion === 'L') {
      // Preferir la casilla de Líbero si está libre
      if (!state.slots[LIB_SLOT]) {
        targetIdx = LIB_SLOT;
      } else {
        // Si ya hay L o está ocupada, buscar cualquier otra casilla libre
        targetIdx = state.slots.findIndex((v, i) => !v && i !== LIB_SLOT);
      }
    } else {
      // Cualquier posición distinta de Líbero puede ir en cualquier casilla salvo la de L
      targetIdx = state.slots.findIndex((v, i) => !v && i !== LIB_SLOT);
    }
    ...
    // 2) Si no hay espacio en cancha, intentar enviarlo a la banca
    if (p.posicion === 'L') {
      toast('No se permiten Líberos en la banca.');
      return;
    }
    if (state.bench.length >= 6) {
      toast('Banca llena (máximo 6).');
      return;
    }
    if (isDuplicate(p.varianteId)) {
    ...
    $$('.droptarget', scope).forEach(target=>{
      target.addEventListener('dragover', e=>{
        ...
        if(posTarget === 'BENCH'){
          ...
          // Solo restringimos la casilla de Líbero: en ese slot solo pueden ir jugadores L.
  ...
  function handleDropSlot(varId, logicalIdx){
    ...
    const needPos = SLOT_LAYOUTS[state.layoutIdx][logicalIdx].pos;
    // Solo restringimos la casilla de Líbero: en ese slot solo pueden ir jugadores L.
    if(needPos === 'L' && p.posicion !== 'L'){ toast('Solo Líberos pueden ocupar esta casilla.'); return; }
