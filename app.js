...
function saveAccordion(){
  ...
  function renderSpecialtyBonuses(){
    ...
    const rows = Object.keys(groups).map(tagKey=>{
      ...
      let descHtml = '';
      if(desc){
        descHtml = '<p class="tb-bonus-desc">' + desc + '</p>';
      }
      ...
      if(active){
        ...
        statusHtml = '<div class="tb-bonus-status active">Activo: ' +
          (stat ? (stat+': ') : '') +
          '<b>+' + val + suffix + '</b></div>';
      }else if(next){
        ...
        const suffix = type === 'porcentaje' ? '%' : '';
        statusHtml = '<div class="tb-bonus-status pending">Te faltan <b>' + need +
          '</b> titulares con <b>' + label + '</b> para activar: ' +
          (stat ? (stat+': ') : '') +
          '+' + next.value + suffix + '</div>';
      }else{
  ...
  function renderPositionBonuses(){
    ...
    const rows = state.positionBonuses.map(b=>{
      ...
      if(active){
        ...
        statusHtml = '<div class="tb-bonus-status pending">Te faltan <b>' + need +
          '</b> titulares en <b>' + pos + '</b> para este tramo.</div>';
      }else{
