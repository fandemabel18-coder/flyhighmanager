...
function renderCompareSkills(){
  ...
  function renderFor(id){
    if(!id) return '<p class="meta">Selecciona un personaje.</p>';
    if(!CHAR_BY_ID || !CHAR_BY_ID.size) return '<p class="meta">Datos no disponibles.</p>';
    const c = CHAR_BY_ID.get(String(id));
    if(!c) return '<p class="meta">Sin datos del personaje.</p>';

    const skills = Array.isArray(c.skills) ? c.skills : [];
    if(!skills.length) return '<p class="meta">Sin datos de habilidades.</p>';

    const items = skills.map(s => {
      if(typeof s === 'string'){
        return `<li class="cmp-skill-item"><div class="cmp-skill-body"><div class="cmp-skill-name">${s}</div></div></li>`;
      }
      const name = s.name || 'Habilidad';
      const eff  = s.effect || s.desc || '';
      return `<li class="cmp-skill-item">
        <div class="cmp-skill-body">
          <div class="cmp-skill-name">${name}</div>
          ${eff ? `<div class="cmp-skill-text">${eff}</div>` : ''}
        </div>
      </li>`;
    }).join('');

    return `<ul class="cmp-skill-list">${items}</ul>`;
  }

  colA.innerHTML = renderFor(COMPARE.A);
  colB.innerHTML = renderFor(COMPARE.B);
}
